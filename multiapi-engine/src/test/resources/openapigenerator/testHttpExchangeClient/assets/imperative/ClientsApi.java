package com.sngular.multifileplugin.httpexchange;
...
import com.sngular.multifileplugin.httpexchange.model.ClientDTO;

/**
 * Clients API, as a Spring HTTP service interface. Back it with a configured {@code RestClient}, whose
...
public interface ClientsApi {
  ...
  ResponseEntity<ClientDTO> getClient(@PathVariable("client_id") Long client_id, @RequestParam(name = "since", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate since, @RequestHeader(name = "X-Correlation-Id", required = false) String xCorrelationId);
  ...
  ResponseEntity<Void> deleteClient(@PathVariable("client_id") Long client_id);
  ...
  ResponseEntity<ClientDTO> createClient(@RequestBody(required = true) ClientDTO clientDTO);
  ...
  ResponseEntity<Void> uploadDocument(@PathVariable("client_id") Long client_id, @RequestPart(name = "file", required = true) MultipartFile file, @RequestPart(name = "comment", required = false) String comment);
}
