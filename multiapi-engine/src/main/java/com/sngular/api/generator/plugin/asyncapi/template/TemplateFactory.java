...
public class TemplateFactory {
  ...
  private void fillTemplate(
    ...
    fileToSave.mkdirs();
    final String pathToSaveMainClass =
        fileToSave.toPath().resolve(className + FILE_TYPE_JAVA).toString();
    fillTemplate(pathToSaveMainClass, templateName, root);
  }

  private void fillTemplate(
      final String pathToSaveMainClass, final String templateName, final Map<String, Object> root)
      throws IOException, TemplateException {
    writeTemplateToFile(templateName, root, pathToSaveMainClass);
  }

  public final void fillTemplates(final boolean generateExceptionTemplate)
      throws IOException, TemplateException {
    ...
    if (!schemaObjectMap.isEmpty()) {
      try {
        fillTemplates(
            schemaObjectMap.get(0).getPropertiesPath(),
            schemaObjectMap.get(0).getModelPackage(),
            propertiesSet);
  ...
  private void fillTemplates(
      final Path filePathToSave, final String modelPackage, final Set<String> fieldProperties)
  ...
  public final void fillTemplateModelClassException(
      final Path filePathToSave, final String modelPackage) throws IOException, TemplateException {
    ...
    final String pathToSaveMainClass =
        pathToExceptionPackage.resolve("ModelClassException.java").toString();
  ...
  public final void fillTemplateCustom(
      final Path filePathToSave,
      final String modelPackage,
      final String fileNameAnnotation,
      final String templateAnnotation,
      final String fileNameValidator,
      final String templateValidator)
      throws TemplateException, IOException {
    ...
    final String pathToSaveAnnotationClass =
        pathToCustomValidatorPackage.resolve(fileNameAnnotation).toString();
    writeTemplateToFile(templateAnnotation, root, pathToSaveAnnotationClass);
    final String pathToSaveValidatorClass =
        pathToCustomValidatorPackage.resolve(fileNameValidator).toString();
    writeTemplateToFile(templateValidator, root, pathToSaveValidatorClass);
  }

  @SuppressWarnings("checkstyle:CyclomaticComplexity")
  private Set<String> fillTemplateSchema(
      final ClassTemplate classTemplate, final Boolean useLombok, final String exceptionPackage)
      throws IOException, TemplateException {
    ...
    if (Objects.nonNull(schemaObject)
        && Objects.nonNull(schemaObject.getFieldObjectList())
        && !schemaObject.getFieldObjectList().isEmpty()) {
      final Map<String, Object> rootSchema = new HashMap<>();
      rootSchema.put("schema", schemaObject);
      root.put("schema", schemaObject);
      final String templateName =
          null != useLombok && useLombok
              ? TemplateIndexConstants.TEMPLATE_CONTENT_SCHEMA_LOMBOK
              : TemplateIndexConstants.TEMPLATE_CONTENT_SCHEMA;
  ...
  public final void addSupplierMethod(final ProcessMethodResult processMethod) {
    publishMethods.add(
        MethodObject.builder()
            .operationId(processMethod.getOperationId())
            .classNamespace(processMethod.getNamespace())
            .type("publish")
            .keyClassNamespace(processMethod.getBindings())
            .bindingType(processMethod.getBindingType())
            .build());
  }

  public final void addStreamBridgeMethod(
      final ProcessMethodResult processMethod, String channelName) {
    streamBridgeMethods.add(
        MethodObject.builder()
            .operationId(processMethod.getOperationId())
  ...
  public final void addSubscribeMethod(ProcessMethodResult processMethod) {
  ...
  public final void fillTemplateWrapper(
    ...
    writeTemplateToFile(
        ...
        context,
        filePath.resolve("MessageWrapper.java").toAbsolutePath().toString());
  }
    ...
    for (MethodObject method : allMethods) {
      ...
      if (Objects.equals(method.getType(), "publish")) {
        fillTemplate(
            supplierFilePath,
            "I"
                + method.getOperationId().substring(0, 1).toUpperCase()
                + method.getOperationId().substring(1),
            checkTemplate(
                method.getBindingType(), TemplateIndexConstants.TEMPLATE_INTERFACE_SUPPLIERS),
            interfaceRoot);
        ...
        fillTemplate(
            ...
            "I"
                ...
                + method.getOperationId().substring(1),
            checkTemplate(
                method.getBindingType(), TemplateIndexConstants.TEMPLATE_INTERFACE_CONSUMERS),
            interfaceRoot);
      }
    }
  }
  ...
  private void writeTemplateToFile(
      final String templateName, final Map<String, Object> root, final String path)
      throws IOException, TemplateException {
  ...
  private String checkTemplate(final String bindingType, final String defaultTemplate) {
    ...
    switch (BindingTypeEnum.valueOf(bindingType)) {
      ...
      case KAFKA:
        templateName =
            MapperUtil.splitName(defaultTemplate)[0] + TemplateIndexConstants.KAFKA_BINDINGS_FTLH;
        break;
      default:
