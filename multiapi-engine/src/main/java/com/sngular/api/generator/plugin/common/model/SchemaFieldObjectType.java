...
public class SchemaFieldObjectType {
  ...
  private static final Map<String, String> IMPL_TYPE_MAPPINGS =
      Map.ofEntries(
          new SimpleImmutableEntry<>(TypeConstants.OBJECT, "Object"),
  ...
  public boolean containsType(final String type) {
    return type.equalsIgnoreCase(baseType)
        || Objects.nonNull(innerType) && innerType.containsType(type);
  }

  private String mapIntoString(final Map<String, String> mappings) {
    final String baseString =
        TypeConstants.OBJECT.equals(baseType) && Objects.nonNull(innerType)
            ? "?"
            : mappings.getOrDefault(baseType, StringUtils.capitalize(baseType));
    final boolean hasInner = baseString.contains("?");

    if (hasInner && Objects.isNull(innerType)) {
      throw new CodeGenerationException(
          String.format("Field object type '%s' missing an inner type", baseType));
    }
  ...
  public boolean equals(final Object obj) {
    ...
    if (obj instanceof SchemaFieldObjectType) {
      ...
      final boolean innerTypeIsEqual =
          Objects.isNull(innerType)
              ? Objects.isNull(other.innerType)
